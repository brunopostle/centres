...
artwork, so any variation a measure shows under ``ISOMETRY`` is measurement
noise with no possible structural interpretation. ``BENIGN`` adds tonal and
codec changes, which likewise leave composition intact. ``PRACTICAL`` adds the
things that happen to real photographs of real objects — resizing, cropping,
lens vignetting, oblique viewpoint — where a small change is defensible but a
large one makes the measure unusable in the field.
"""
